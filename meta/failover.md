...
4. 仅当今天缺稿且 Neon 仍是 active primary 时，先按下述租约协议 CAS claim，成功后才进入完整日报流水线。
5. 06:00 之后，只要存在 active standby，primary 不得与 standby 并发 self-heal。

## standby failover — Cream（06:00）

standby 任务只做检查；若今日已成功，立即结束。Cream（order 2）仅在 **06:00** 启动。若仍缺稿，开始任何论文检索前必须取得 ownership：
...
到窗口末端，当前 Bot 停止生成及发布；即使没有成功写入 release，时间到期也会解除占用。下一层在自己的窗口检查并 CAS 替换过期租约；例如 Neon 05:30 认领后卡死，Cream 06:00 可接管。
