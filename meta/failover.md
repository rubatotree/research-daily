...
## standby failover — Cream（名义 06:20；平台 cron 06:00）
...
到窗口末端，当前 Bot 停止生成及发布；即使没有成功写入 release，时间到期也会解除占用。下一层在自己的窗口检查并 CAS 替换过期租约；例如 Neon 在窗口内认领后卡死，Cream 06:20 可接管。
