...
## 2026-09-26 · 当日校准摘要（notes · 无 Cream 私聊）

自上次兴趣同步以来，**未**与 Cream 新增可公开兴趣讨论。按 `research-interests.md`「notes 与 blog 联合校准」扫到：

1. **阅读加权：已知运动学约束下的同轨迹动态高斯重放 / 新视角一致性** — 详见下方专题。来自 notes 仓 2026-09-26 开题写作收敛（tip `3851764`→`0630f94`）；概念层动机与阅读线索，未验证，不替换 MiracleAug 工程主线。
2. **blog / academic：** blog tip 仍为 `8d722e0`（SA 2026 论文笔记草稿未变，仍勿引用草稿正文）；academic 无新公开发布。

---

## 2026-09-26 · 已知运动学 × 同轨迹新视角重放（阅读兴趣 · 轻加权）

- **触发：** notes 仓 2026-09-26 记录维护者开题写作收敛到一类可公开跟读的问题族：共享规范场景与稀疏同步相机下，**已知机器人运动学**能否改善**同一段已记录操作**的新视角渲染与运动一致性（同轨迹重放）。新物体布局与新动作轨迹合成**不是**该收敛下的基本目标表述。
- **公开题目口径：** 《面向具身智能应用的 4D 高斯泼溅场景重建》 / *4D Gaussian Splatting Scene Reconstruction for Embodied AI Applications*（此处 4D 指时变三维高斯场景表示，不等于绑定某一特定方法专名）。
- **性质：** 研究动机 / 阅读加权。**不是**已证实结论，**不**把未公开开题正文、评审意见或实验协议写成已开展结果。
- **日报怎么用：** 「新兴趣相关」可轻扫公开对照族——运动学/结构驱动的操作场景动态高斯（如 [ManiSplat](https://arxiv.org/abs/2606.10645)）、示教资产复用与编辑后重规划（如 [RoboSplat](https://yangsizhe.github.io/robosplat/)（[arXiv:2504.13175](https://arxiv.org/abs/2504.13175)））、控制点/骨架驱动动态高斯（SC-GS、SV-GS）等；点明与卡点①拆分绑定、MiracleAug 可编辑动态资产的交叉。写稿时区分「同轨迹新视角重放」与「新轨迹合成 / 静态编辑后重规划」。勿写「首次」、勿编造本地实验。
- **禁止写入：** notes / 开题草稿正文、评审意见细节、未公开实验设计与任务细节、资源额度、私人排期、导师/教务登记状态。
- **状态：** active · reading · light-weight · from-notes-calibration · tip=`0630f94`
