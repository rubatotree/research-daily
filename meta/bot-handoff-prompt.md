...
- 每天按 failover.json 中自己的角色与窗口运行；primary 名义 05:30（平台 cron 05:10，窗至 06:20）、standby 06:20（Asia/Hong_Kong）；详见 failover.json 晚唤醒补偿。
...
- [ ] 试跑不把额度、路径、密钥写进 diff
- [ ] 按名册角色启用定时：primary `10 5 * * *`；standby `20 6 * * *`；兴趣同步 `45 4 * * *`
