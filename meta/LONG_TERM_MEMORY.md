...
6. **图文一体（2026-09-23）：** 论文含 pipeline（方法流程）图时，优先展示原论文 pipeline 图并附图号与来源链接；只有论文没有 pipeline 图，或原图不够清晰、确有必要改善原理展示时，才补充自绘示意图。自绘图尽量使用中文标注，必要的模型名、缩写和公式保留原文；标明「据论文整理的示意图」及依据，不冒充原图。 必读文尽量嵌图，原图配中文解读；自绘仅依据公开资料。
7. **中文可读、学术准确：** 作者不中译姓名；单位仅在确定时用中文机构名
8. **名词解释与方法深度（2026-09-22）：** 每期独立可读，陌生名词/缩写首次使用即解释含义及本文作用，模型与基准先说明身份。重点讲清「问题与瓶颈 → 输入/输出/前提 → 关键步骤及数据流 → 为什么有效 → 训练与部署区别」，必要时用逐项解释的核心公式或具体例子。先讲方法，再讲实验、成本与边界；篇幅不足时少收论文，不把方法压缩成术语清单。全文依据、术语和指标自检按 `digest-spec.md`「可读性与方法讲解」执行。
...
- 2026-09-27：兴趣同步（Cream）据 notes tip `0630f94` 联合校准，公开写入「已知运动学 × 同轨迹新视角重放」阅读加权（不复制开题/评审正文；blog tip 仍 `8d722e0`）。

- 2026-09-24：兴趣同步（Cream）据 notes/blog 联合校准，公开写入 VLA 过程监督阅读加权与 SA 2026 论文笔记草稿（仅阅读线索，不复制 notes/草稿正文）。
...
- 2026-09-22：加强日报可读性；名词须就地解释，方法与原理必须形成完整因果链，保留数学和实现深度；同步规范与接管文案，并改写当期示例。
