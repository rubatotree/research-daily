...
**圈内观点。** 量子位/机器之心周末稿不只是发模型：HiDream 把鲁棒性押在「不完美观测训练 + 生成扩数据」；空间智能长文则明确——3DGS/点云重建把世界拍真仍是上半场，**推演与决策**才是尺子（详见下文「圈内动态」）。
...
覆盖约 **2026-09-07 HKT**（及周末仍在发酵的具身/世界模型信号）。只收与图形 / 3DGS / 具身相关的高信号项；每条补**观点提炼**（非标题搬运）。

### 中文媒体 / 公众号

**量子位 · HiDream-O1-Embodied**（2026-09-07）  
- **事实：** 智象（HiDream.ai）发具身世界模型，宣称打通图像/视频/3D/动作，并在 RoboColiseum「扰动适应」子榜以 0.692 登顶；文为厂商供稿转载。  
- **观点：** CTO 姚霆把世界模型基座压成三件事——全模态表达、因果推演、物理世界构建；执行侧强调语言意图（而非关键词）、多视角互补（避免单相机挂掉）、训练期主动引入不完美观测。数据叙事是「真实 mocap 基座 + 生成增强」飞轮（与诺亦腾合作扩样本）。  
- **对照：** 和本刊 Real2Sim / 示教数据荒同一张桌子——他们用生成扩扰动，我们更关心可编辑资产与可重光照几何是否撑得住下游控制。  
- 链：https://www.qbitai.com/2026/09/485056.html

**机器之心 · 同题报道**（2026-09-07）  
- 叙事与上条同向（理解→推演→执行闭环）；可作交叉确认，观点增量有限。  
- 链：https://c.m.163.com/news/a/L67RHSK10511AQHO.html

**机器之心 · Human-Centric AI 综述**（2026-09-06）  
- **事实：** 推介综述 *Human-Centric Intelligence in the Era of Foundation Models*（arXiv:2608.18184），把以人为中心的工作收成六层谱系。  
- **观点：** 从「看见人」走到「行动如人」——外观/几何 → 运动/交互 → 世界模拟 → 具身行动；作者强调下一步不只堆规模，还要结构可信、物理可行、行动有用。世界模型段点出：从通用视频先验转向以人为条件的世界演化，看重一致、因果与长时可控。  
- **对照：** 绑骨/蒙皮与 4D 人–场景一致（本报 ARAP-GS / InverseRig / CC-4DGS 一线）正好落在「运动–交互–世界」中间几层。  
- 链：https://www.163.com/dy/article/L65GM2Q90511AQHO.html · 综述 https://arxiv.org/abs/2608.18184

**量子位 · 世界模型与空间智能**（约 2026-09-05）  
- **事实：** 以飞渡科技 / HICOOL 与投资人 Bill Reichert 为切口，讨论「LLM 缺物理世界通路」。  
- **观点（高信号）：** 生成逼真 3D/视频仍是上半场；下半场尺子是**推演与决策**。文中点名 Atlas 一类从照片重建点云与 **3DGS** 的路线「把世界拍得更真」，但防灾/调度要的是可验证推演——中间还隔着仿真、动态边界与验证体系。评测（TriWorldBench、WorldArena）也在从「好不好看」拧向「能不能用」。  
- **对照：** 与 PTIR-GS「真实感 vs 管线可用」同构：好看的重建不等于可仿真、可批渲染、可闭环的资产。  
- 链：https://www.qbitai.com/2026/09/484683.html

### X
本日未做高成本关键词深挖；动态 GS / 压缩相关条目流可延续昨日 CSVisionPapers 对 CC-4DGS 等的转发线索。后续有高信号帖时按「事实 / 观点 / 对照」三条写，避免只转链。
