...
**本日未确认值得单列的高信号中文媒体条目。** 不以厂商宣传补足篇数。

[Ke-Sen 主索引](https://kesen.realtimerendering.com/)目前仍未列出 SIGGRAPH Asia 2026 专页，但这不代表相关论文尚未公开。本期 RelightFormer 的会议信息由论文与作者仓库确认；集合页是否上架与单篇是否录用，应分开判断。

今天留下两个具体问题：**FIRE3D 的后处理成本还能怎样批量化？Pose6DAug 的对象级失败选择怎样扩展到其他可控扰动？** 前者关注单位资产成本，后者关注单位数据价值；二者都比单纯增加模型规模或样本数量更适合继续拆解。
