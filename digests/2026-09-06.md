...
> 说明：v6 在 v5 全字段基础上为每篇补 **方法概要**（据 HTML/PDF，非摘要复述）与 **值得关注**（贴 PTIR-GS / Real2Sim；刘利刚组单独点名；写清限制）。正文互引一律锚点链接。摘要经 arXiv API 复核后中译；链接经 HTTP / abs / 项目页校验。Ke-Sen SIGGRAPH 2026 合集 `Last-Modified: 2026-08-20 01:37:43 GMT`（≈ **2026-08-20 09:37 HKT**），近窗无增量。作者名保留英文 / 拼音；国内明确单位用中文。发表时间取 arXiv 首次挂出日；已接收则同时写 venue。
...
- **新智元 / PaperWeekly**：本 run 主站抓取失败或无稳定公开列表；未编造条目。

### X（关键词检索）
余额约 **$11.87**；先 `get_posts_counts_recent`（$0.005），再 `search_posts_all` max_results=10（约 $0.05–0.15 量级，含 author 展开）。高信号摘录：
...
| 条目 | 为何放这里 | 链接 |
|------|------------|------|
| **G-Skin**（arXiv:2608.01726） | 生成式视觉先验学 3DGS 蒙皮权重 + ARAP/结构耦合正则；与 [GradRig](#gradrig) 分工（学权重 vs 用权重梯度拉伸）。**非本窗口新文。** | https://arxiv.org/abs/2608.01726 · https://yaoyx689.github.io/GSkin.html |
| **F-RNG**（arXiv:2605.25975） | 在 LRM + 本征分解先验上前馈出可重光照神经高斯；与 [LightBridge](#lightbridge) / PTIR 同桌比「前馈可重光照」路线。**非本窗口。** | https://arxiv.org/abs/2605.25975 |
| **MVInverse**（CVPR 2026） | 多视角前馈联合几何与材质（albedo/metal/roughness 等），秒级；对照逐场景逆渲染。**非本窗口。** | https://openaccess.thecvf.com/content/CVPR2026/papers/Wu_MVInverse_Feed-forward_Multiview_Inverse_Rendering_in_Seconds_CVPR_2026_paper.pdf |
...
- v6 主文 **14** 篇高信号全格式条目，均补方法概要 + 值得关注；站外补充 4 条入「新兴趣相关」
- **命名规则：** 作者名保留英文 / 拼音；仅对明确国内单位使用中文；其余单位用原文
- **发表时间：** 一律取 arXiv `<published>` 首次挂出日；已接收则同时写 venue
- **链接获取失败 / 受限：** LightBridge / AnyGS2Mesh / TileGS / NavArena / UE-EchoWM 未见已验证公开主页或代码；PointGT / P-CORE 有项目页与视频、未见代码仓；新智元 / PaperWeekly 本 run 无稳定抓取
- **配图：** 正文相对路径 `2026-09-06/figs/...`
- **X：** 有额度；本 run 仅廉价 counts + 单页 10 条搜索，未做多页翻检

---

*策展版 v6 生成：2026-09-08 ≈ 00:40 HKT · 兴趣校准：rubatotree blog Real2Sim/SC-GS + PTIR-GS · 方法据 arXiv HTML · 圈内：量子位/机器之心/Ke-Sen/X · 已去标签、锚点互引*
